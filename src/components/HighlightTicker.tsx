...
const fallbackAnnonces: Annonce[] = [
  {
    id: "fallback1",
...
const HighlightTicker = () => {
  const [highlights, setHighlights] = useState<
    { id: string; title: string; slug: string; type: "article" | "annonce" }[]
  >([]);
  ...
  useEffect(() => {
    const fetchHighlights = async () => {
      try {
        ...
        // fallback
        const fallback = [
          ...fallbackArticles.map((a) => ({ id: a.id, title: a.title, slug: a.slug, type: "article" as const })),
          ...fallbackAnnonces.map((a) => ({ id: a.id, title: a.titre, slug: a.lien, type: "annonce" as const })),
        ];
        ...
        {highlights.slice(0, 4).map((highlight) => {
          ...
          return (
            <Highlight key={highlight.id}>
              <HighlightTitle>
                <Link to={link}>{highlight.title}</Link>
              </HighlightTitle>
            </Highlight>
          );
        })}
